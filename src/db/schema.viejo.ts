import { integer, pgTable, serial, timestamp, varchar, boolean, pgEnum, date, unique } from "drizzle-orm/pg-core";
...
export const employeeFamilyTable = pgTable("employeeFamily", {
    id: serial().primaryKey(),
    employeeId: integer().notNull().references(() => employeeTable.id, { onDelete: 'cascade' }),
    familyId: integer().notNull().references(() => familyTable.id, { onDelete: 'cascade' }),
    createdAt: timestamp().defaultNow(),
    updatedAt: timestamp().defaultNow()
}, (table) => { // 'table' representa la definición de la tabla
    return {
      uniqueEmployeeFamily: unique("employee_family_unique").on(table.employeeId, table.familyId),
    };
  }
);